...
def bfs_array(data: Any) -> Generator[np.ndarray]:
    ...
    ```
    """
    state = IteratorState(iterator=ArrayIterator(), queue=deque([data]))
    while state.queue:
...
class ArrayIterator(BaseArrayIterator[Any]):
    ...
    def add_iterator(
        ...
        ```pycon
        >>> from batcharray.utils.bfs import ArrayIterator, IterableArrayIterator
    ...
    def has_iterator(cls, data_type: type) -> bool:
        ...
        ```pycon
        >>> from batcharray.utils.bfs import ArrayIterator
