...
def bfs_array(data: Any, registry: IteratorRegistry | None = None) -> Generator[np.ndarray]:
    ...
    ```pycon
    >>> import numpy as np
    >>> from batcharray.utils.bfs2 import bfs_array
...
class IteratorRegistry:
    ...
    ```pycon
    >>> from batcharray.utils.bfs2 import IteratorRegistry, IterableArrayIterator
    >>> import numpy as np
    ...
    def has_iterator(self, data_type: type) -> bool:
        ...
        ```pycon
        >>> from batcharray.utils.bfs2 import IteratorRegistry
        >>> registry = IteratorRegistry()
    ...
    def find_iterator(self, data_type: type) -> BaseArrayIterator:
        ...
        ```pycon
        >>> from batcharray.utils.bfs2 import get_default_registry
        >>> registry = get_default_registry()
...
def register_iterators(mapping: Mapping[type, BaseArrayIterator], exist_ok: bool = False) -> None:
    ...
    ```pycon
    >>> from batcharray.utils.bfs2 import register_iterators, IterableArrayIterator
